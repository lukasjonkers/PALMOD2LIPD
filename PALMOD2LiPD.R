...
PALMOD2LiPD <- function(x, path = "dataLiPD/"){   # x is list with data
  ...
  #will use this later.
  createTSid <- function(){
    return(paste(c("PM",sample(c(letters,LETTERS,seq(0,9)),size = 10,replace=TRUE)),collapse = ""))
  }
  ...
  if(!is.null(chronData)){ # catch cases without chronology data
    ...
    for(v in 1:length(cdc)){
      if(any(!is.na(chronData[[cdc[[v]]]]))){
        ...
        if(is.logical(mt[[i]]$values)){
          mt[[i]]$values <- as.character(mt[[i]]$values)
        }
        mt[[i]]$TSid <- createTSid()
  ...
  #yes
  L$paleoData[[1]]$model  <- map(e$SurfaceTempEnsembles, function(x){
    ...
    lis <- list(
      ...
      ensembleTable = vector(mode = "list",length = 1)
      )
    
    lis$ensembleTable[[1]] <- list(
      depth = list(
      ...
      sstEnsemble = list(
        values =  as.matrix(x$ensembles[,-1]),
        TSid = createTSid(),
  ...
  allDOIs <- unique(c(pubDOIs, chronDOIs))
